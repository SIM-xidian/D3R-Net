...
Gaussian diffusion model is adapted from ILVR_ADM: https://github.com/jychoi118/ilvr_adm
"""
...
import torch
from D3R.utils import mean_flat
from D3R.loss import normal_kl, discretized_gaussian_log_likelihood
from D3R.resizer import Resizer
from D3R.rmse import rmse
...
class GaussianDiffusion:
    ...
    :param model_mean_type: a ModelMeanType determining what the model outputs.
                            0: noise; 1:x_0; 2:x_{t-1}
    :param model_var_type: a ModelVarType determining how variance is output.
                            0:learned; 1:fixed_small; 2:fixed_large; 3:learned_range
    ...
    def q_mean_variance(self, x_start, t):
        ...
        log_variance = _extract(self.log_one_minus_alphas_cumprod, t,
                                x_start.shape)
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        a = _extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        b = _extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
    ...
    def d3r_sample_loop(
        ...
        total_noise_level = th.tensor([start_step] * shape[0],
                                      device=self.device)

        # Initial Condition
        img = self.q_sample(ref_img, total_noise_level, noise)
        ...
        for i in range(start_step, -1, -1):
            ...
            with th.no_grad():
                ...
                # Truncated Output
                if i == output_step:
                    pred_x_0 = out["pred_xstart"]
                    return pred_x_0

                # # DFCM
                if resizers:

                    n = math.floor(rmse(img, self.q_sample(ref_img, t - 1)))
                    down_N = math.pow(2, n)
                    shape = (1, 3, img_size, img_size)
                    shape_d = (1, 3, img_size // down_N, img_size // down_N)
                    ...
                    # img = img - up(down(self.q_sample(ref_img, t - 1))) + up(down(img))
                else:
                    img = self.q_sample(ref_img, t - 1)
    ...
    def p_mean_variance(self,
        ...
        if model_output is None:
            if cond is None:
                ...
                model_output = self.model(th.cat((x, cond), 1),
        ...
        if self.model_var_type in [
            ...
            if self.model_var_type == ModelVarType.LEARNED:
                ...
                min_log = _extract(self.posterior_log_variance_clipped, t,
                                   x.shape)  ##############
            ...
            model_variance = _extract(model_variance, t,
                                      x.shape)  ##############
            model_log_variance = _extract(model_log_variance, t,
                                          x.shape)  #################
    ...
    def _predict_xstart_from_xprev(self, x_t, t, xprev):
        ...
        return (  # (xprev - coef2*x_t) / coef1
                _extract(1.0 / self.posterior_mean_coef1, t, x_t.shape) * xprev -
                _extract(self.posterior_mean_coef2 / self.posterior_mean_coef1, t,
                         x_t.shape) * x_t)

    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        return (_extract(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t -
                pred_xstart) / _extract(self.sqrt_recipm1_alphas_cumprod, t,
                                        x_t.shape)
    ...
    def ddim_sample(
        ...
        alpha_bar = _extract(self.alphas_cumprod, t, x.shape)
        alpha_bar_prev = _extract(self.alphas_cumprod_prev, t, x.shape)
    ...
    def ddim_reverse_sample(
        ...
        # Usually our model outputs epsilon, but we re-derive it
        # in case we used x_start or x_prev prediction.
        eps = (_extract(self.sqrt_recip_alphas_cumprod, t, x.shape) * x -
               out["pred_xstart"]) / _extract(self.sqrt_recipm1_alphas_cumprod,
                                              t, x.shape)
        alpha_bar_next = _extract(self.alphas_cumprod_next, t, x.shape)
    ...
    def ddim_sample_loop(
        ...
        if progressive:
            res = []
            trace_step = self.num_timesteps // 10
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)),
            ...
            with th.no_grad():
                ...
                img = out["sample"]
                if progressive and not (self.num_timesteps - i) % trace_step:
                    res.append(img)
    ...
    # Training
    def _vb_terms_bpd(self, model_output, x_start, x_t, t, clip_denoised=True):
        """
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        # def training_losses(self, x_start, t, model_kwargs=None, ref=None, ref_deg=False, noise=None):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if ref is None:
                ...
                if ref_deg:
                    old_t = t[0].item()
                    tt = old_t // 10 if old_t > 200 else 20
                    tt = th.tensor([tt] * x_t.size(0)).long().to(self.device)
                    ref = self.q_sample(ref, tt)
                model_output = self.model(th.cat((x_t, ref), 1), t)
            ...
            if self.model_var_type in [
                ...
                # The output of self.model has 6 channel instead of 3
                # To learn the variance using the variational bound,
